...
def generate_report_text(project_name: str, project_type: str, company_name: str, total_budget: float, allocations: dict, total_pct: float) -> dict:
    ...
    estimate_summary = (
        ...
        f"{company_name} has prepared a preliminary budget estimate with a total projected value of {total_budget:,.2f}. "
        ...
        f"The current model reflects an expected profit margin of {profit_pct:.2f}% with an estimated profit value of {profit_amount:,.2f}. "
    ...
    disclaimer_note = (
        f"This budget estimate is prepared solely for planning, review, and preliminary commercial evaluation purposes. "
        f"Final values may vary depending on site conditions, actual quantities, client requirements, specification changes, transportation conditions, authority approvals, and material price fluctuations at the time of execution. "
        f"Any variation in scope, timeline, procurement conditions, or execution methodology may lead to revision of the final commercial offer and agreement terms."
    )
    ...
    internal_note = (
        ...
        f"Management is advised to review category distribution, execution risks, and contingency adequacy before final approval or client submission."
    )
...
def create_excel_report(project_name: str, project_type: str, company_name: str, author_name: str, today: str, total_budget: float, allocations: dict, df: pd.DataFrame, insights: list[str]) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        # Write raw tables lower down so we can format premium sections above
        ...
        note_text = (
            ...
            f"Current allocation status is {allocation_status.lower()}, with an estimated profit of {profit_pct:.2f}% "
            f"and profit amount of {profit_amount:,.2f}."
        )
        ...
        for col, width in {"A": 24, "B": 16, "C": 18, "D": 18}.items():
        ...
        bd[f"B{total_row}"] = total_allocation_pct / 100
        ...
        for row in range(5, 5 + len(insights_with_status)):
            ...
            ins[f"A{row}"].alignment = Alignment(wrap_text=True, vertical="top")
            val = str(ins[f"A{row}"].value)
            if "✅" in val or "Balanced" in val:
                ins[f"A{row}"].fill = green_fill
            elif "⚠️" in val or "❌" in val or "Over" in val or "Under" in val:
        ...
        pie = PieChart()
        labels = Reference(ch, min_col=1, min_row=3, max_row=2 + len(df))
        data = Reference(ch, min_col=2, min_row=2, max_row=2 + len(df))
...
report_option = st.selectbox(
    "Select Report Draft Type",
    options=list(report_sections.keys()),
)

st.markdown("### Generated Draft")
st.text_area(
    "Report Text",
    value=report_sections[report_option],
    height=220,
)
