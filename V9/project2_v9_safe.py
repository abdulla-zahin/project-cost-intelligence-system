...
def clean_boq_dataframe(raw_df: pd.DataFrame) -> pd.DataFrame:
    missing_columns = [col for col in REQUIRED_BOQ_COLUMNS if col not in raw_df.columns]
    if missing_columns:
        raise ValueError(f"Missing required BOQ columns: {', '.join(missing_columns)}")

    boq_df = raw_df.copy().dropna(how="all")
...
st.caption("Upload a BOQ in CSV or XLSX format with columns: Item, Quantity, Unit Cost, Category")
